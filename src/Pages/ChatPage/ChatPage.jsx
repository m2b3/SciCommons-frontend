...
const ChatPage = () => {
    ...
    const getArticle = async () => {
        ...
        try {
            const res = await axios.get(`https://scicommons-backend.onrender.com/api/article/${id}`,config);
    ...
    const handleSubmit = async() => {
        ...
        if(Message.body !== "")
        {
            ...
            const response = await axios.post(`https://scicommons-backend.onrender.com/api/article_chat/`,{
                article: id,
                body: Message,
            }, config );
      ...
      const loadData = async(res) => {
        const messages = [...Messages,{id:res.id, body:Message, sender: user.username, personal: true, created_at: new Date().toISOString(),media: null, article: id}]
        ...
        <> 
            ...
            {!loading && Messages!==null && article!==null && 
                <div className="flex h-screen">
                    ...
                    <div style={{cursor:"pointer"}} className="flex items-center xs:text-xs sm:text-sm md:text-3xl p-3 shadow-xl z-10 bg-white text-gray-600 font-bold flex flex-row justify-start sticky top-0">
                        <span onClick={()=>{navigate(`/article/${article.id}`)}} style={{cursor:"pointer"}} ><BiLeftArrowAlt size={30}/></span>
                        <span className="ml-3 flex">{article.article_name.length > 30?(article.article_name.slice(0,30).replace(/_/g, " ")+"..."):article.article_name.replace(/_/g, " ")}</span>
                    </div>
