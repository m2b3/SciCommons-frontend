...
const Suggestions = ({ suggestions }) => {
    ...
    const handleClick = (e,index) => {
        e.preventDefault();
        const channelName = suggestions[index].username <= user.username.toString() ? `chat_${suggestions[index].username}_${user.username}` : `chat_${user.username}_${suggestions[index].username}`;
      ...
      <ul className="mt-2 border border-gray-300 rounded-md bg-white shadow-md">
        {suggestions.map((suggestion, index) => (
          <li key={index} className="px-4 py-2 hover:bg-gray-100 cursor-pointer">
            <div onClick={(e)=>{handleClick(e,index)}} className="flex flex-row">
                {suggestion.profile_pic_url.includes("None") ?<SlUser className="w-6 h-6"/> :<img src={suggestion.profile_pic_url} className="w-8 h-8 mr-2"/>}
  ...
  const handleSearch = async(e) => {
    if (e.key === 'Enter') {
        ...
        try{
            ...
            const response = await axios.get("/api/user/",{params:{search:e.target.value}}, config);
