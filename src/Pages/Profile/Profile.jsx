...
const Following = ({setFollowingModal, User}) => {
    ...
    if(!followers[index].isFollowing){
      try {
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_URL}/api/user/follow/`,{followed_user:followers[index].user},config)
      ...
      try {
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_URL}/api/user/unfollow/`,{followed_user:followers[index].user},config)
          let updatedFollowers = [...followers];
          updatedFollowers[index].isFollowing = !updatedFollowers[index].isFollowing;
          await loadData(updatedFollowers);
      }
  ...
  const fetchFollowers = async () => {
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/user/following/`, config);
...
const Followers = ({setFollowersModal, User}) => {
  ...
  const fetchFollowers = async () => {
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/user/followers/`, config);
          await loadData(res.data.success)
    }
    catch(err){
        console.log(err)
...
const Profile = () => {
  ...
  const handleFollow = async(e) => {
    ...
    if(!User.isFollowing){
      try {
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_URL}/api/user/follow/`,{followed_user:User.id},config)
      ...
      try {
        const res = await axios.post(
          `${process.env.REACT_APP_BACKEND_URL}/api/user/unfollow/`,{followed_user:User.id},config)
        let updatedUser = {...User};
        updatedUser.isFollowing = !updatedUser.isFollowing;
        updatedUser.followers = updatedUser.followers - 1;
        await loadUserData(updatedUser);
  ...
  const getPosts = async() => {
    ...
    try{
        const res = await axios.get(`${process.env.REACT_APP_BACKEND_URL}/api/user/${username}/posts/`, config)
        await loadData(res.data.success)
    } catch(err) {
        console.log(err)
    }
  ...
  const fetchArticles = async () => {
    ...
    try {
        const response = await axios.get(
            `${process.env.REACT_APP_BACKEND_URL}/api/user/${username}/articles/`,
            config
        );
  ...
  const fetchUser = async() => {
    ...
    try {
      const res = await axios.get(
        `${process.env.REACT_APP_BACKEND_URL}/api/user?search=${username}`,config)
