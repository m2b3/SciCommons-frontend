...
const ContributionsPage: React.FC<MyContributionsClientProps> = ({ initialTab = null }) => {
  ...
  /* Fixed by Codex on 2026-02-09
     ...
     How: Initialize from the server-provided tab param and keep it in sync */
  const [activeTab, setActiveTab] = useState<TabType>(
    isValidTab(initialTab) ? initialTab : 'articles'
  );
  ...
  const handleTabChange = (tab: TabType) => {
    setActiveTab(tab);
    router.replace(`/mycontributions?tab=${tab}`);
  };
