...
const CommunityStats: React.FC<CommunityStatsProps> = ({ community }) => {
  /* Fixed by Codex on 2026-02-15
     Problem: Community stats used fixed gray utilities and skeleton colors.
     Solution: Replace hard-coded colors with semantic tokens for text and placeholders.
     Result: Stats panel and skeleton now follow skin palettes. */
  return (
    <div className="rounded-xl border border-common-contrast bg-common-cardBackground p-4">
      <h2 className="mb-4 font-semibold text-text-primary res-heading-xs">Community Stats</h2>
...
export const CommunityStatsSkeleton: React.FC = () => {
  return (
    <div className="rounded-md bg-common-cardBackground p-4">
      <div className="mb-4 h-6 w-48 animate-pulse rounded bg-common-contrast" />
      <div className="flex flex-col space-y-4">
        {[Users, FileText, UserCheck, FileText].map((Icon, index) => (
          <div key={index} className="flex items-center">
            <Icon className="mr-2 h-5 w-5 text-text-tertiary" />
            <div className="h-4 w-32 animate-pulse rounded bg-common-contrast" />
          </div>
        ))}
      </div>
    </div>
